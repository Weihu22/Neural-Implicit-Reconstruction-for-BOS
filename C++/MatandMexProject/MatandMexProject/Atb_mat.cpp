...
int main() {
	...
	const char* filename = "before_reconstruction_data_forVP.mat";
	...
	bool pseudo_matched = false;
	bool pseudo_mask = false;
	/* copy the string data from prhs[0] into a C string input_ buf.    */
	char* ptype = mxArrayToString(ptypeArray);
	if (!strcmp(ptype, "matched") ) {
		pseudo_matched = true;
	}
	else if (!strcmp(ptype, "mask")) {
		pseudo_mask = true;
	}
	

	// First input should be x from (Ax=b), or the image.
	mxArray const * const projArray = matGetVariable(matFile, "projections");
	if (projArray == nullptr) {
	...
	const mwSize *size_proj = mxGetDimensions(projArray);
	...
	printData(numDims, size_proj, geo, diffselect, ptype, gpuids);
	...
	filename = "Backmask.mat";
	fullpath = std::string(filepath) + filename;
	file = fullpath.c_str();
	saveToMatFile(file, plhs[0], "maskback");
