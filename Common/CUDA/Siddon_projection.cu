...
do { \
        ...
        if (__err != cudaSuccess) { \
				mexPrintf("Error: %s\n", msg);\
				mexPrintf("Ax:Siddon_projection: %s\n", cudaGetErrorString(__err)); \
        } \
} while (0)
...
__global__ void kernelPixelDetector(Geometry geo,
	...
	case 1:
		for (unsigned long ii = 0; ii < Np; ii++) {
			if (ax == aminc) {
				if (i > 0 && i < geo.nVoxelX - 1) {
					sum += (ax - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i - 1, j, k)) / (2.0f * geo.dVoxelX);
				}
				else if (i == 0) {
					sum += (ax - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i, j, k)) / geo.dVoxelX;
				}
				else { 
					sum += (ax - ac)* (tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i - 1, j, k)) / geo.dVoxelX;
				}
				// sum += (ax - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i - 1, j, k)) / 2 / geo.dVoxelX;
				i = i + iu;
				ac = ax;
				ax += axu;
			}
				...
				if (i > 0 && i < geo.nVoxelX - 1) {
					sum += (ay - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i - 1, j, k)) / (2.0f * geo.dVoxelX);
				}
				else if (i == 0) {
					sum += (ay - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i, j, k)) / geo.dVoxelX;
				}
				else {
					sum += (ay - ac)* (tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i - 1, j, k)) / geo.dVoxelX;
				}
				//sum += (ay - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i - 1, j, k)) / 2 / geo.dVoxelX;
				j = j + ju;
				ac = ay;
				ay += ayu;
			}
				...
				if (i > 0 && i < geo.nVoxelX - 1) {
					sum += (az - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i - 1, j, k)) / (2.0f * geo.dVoxelX);
				}
				else if (i == 0) {
					sum += (az - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i, j, k)) / geo.dVoxelX;
				}
				else {
					sum += (az - ac)* (tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i - 1, j, k)) / geo.dVoxelX;
				}
				//sum += (az - ac)*(tex3D<float>(tex, i + 1, j, k) - tex3D<float>(tex, i - 1, j, k)) / 2 / geo.dVoxelX;
				k = k + ku;
				ac = az;
				az += azu;
			}
	...
	case 2:
		for (unsigned long ii = 0; ii < Np; ii++) {
			if (ax == aminc) {
				if (j > 0 && j < geo.nVoxelY - 1) {
					sum += (ax - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j - 1, k)) / (2.0f * geo.dVoxelY);
				}
				else if (j == 0) {
					sum += (ax - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j, k)) / geo.dVoxelY;
				}
				else {
					sum += (ax - ac)*(tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i, j - 1, k)) / geo.dVoxelY;
				}
				//sum += (ax - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j - 1, k)) / 2 / geo.dVoxelY;
				i = i + iu;
				ac = ax;
				ax += axu;
			}
				...
				if (j > 0 && j < geo.nVoxelY - 1) {
					sum += (ay - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j - 1, k)) / (2.0f * geo.dVoxelY);
				}
				else if (j == 0) {
					sum += (ay - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j, k)) / geo.dVoxelY;
				}
				else {
					sum += (ay - ac)*(tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i, j - 1, k)) / geo.dVoxelY;
				}
				//sum += (ay - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j - 1, k)) / 2 / geo.dVoxelY;
				j = j + ju;
				ac = ay;
				ay += ayu;
			}
				...
				if (j > 0 && j < geo.nVoxelY - 1) {
					sum += (az - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j - 1, k)) / (2.0f * geo.dVoxelY);
				}
				else if (j == 0) {
					sum += (az - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j, k)) / geo.dVoxelY;
				}
				else {
					sum += (az - ac)*(tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i, j - 1, k)) / geo.dVoxelY;
				}
				//sum += (az - ac)*(tex3D<float>(tex, i, j + 1, k) - tex3D<float>(tex, i, j - 1, k)) / 2 / geo.dVoxelY;
				k = k + ku;
				ac = az;
				az += azu;
			}
	...
	case 3:
		for (unsigned long ii = 0; ii < Np; ii++) {
			if (ax == aminc) {
				if (k > 0 && k < geo.nVoxelZ - 1) {
					sum += (ax - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k - 1)) / (2.0f * geo.dVoxelZ);
				}
				else if (k == 0) {
					sum += (ax - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k)) / geo.dVoxelZ;
				}
				else {
					sum += (ax - ac)*(tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i, j, k - 1)) / geo.dVoxelZ;
				}
				//sum += (ax - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k - 1)) / 2 / geo.dVoxelZ;
				i = i + iu;
				ac = ax;
				ax += axu;
			}
				...
				if (k > 0 && k < geo.nVoxelZ - 1) {
					sum += (ay - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k - 1)) / (2.0f * geo.dVoxelZ);
				}
				else if (k == 0) {
					sum += (ay - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k)) / geo.dVoxelZ;
				}
				else {
					sum += (ay - ac)*(tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i, j, k - 1)) / geo.dVoxelZ;
				}
				//sum += (ay - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k - 1)) / 2 / geo.dVoxelZ;
				j = j + ju;
				ac = ay;
				ay += ayu;
			}
				...
				if (k > 0 && k < geo.nVoxelZ - 1) {
					sum += (az - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k - 1)) / (2.0f * geo.dVoxelZ);
				}
				else if (k == 0) {
					sum += (az - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k)) / geo.dVoxelZ;
				}
				else {
					sum += (az - ac)*(tex3D<float>(tex, i, j, k) - tex3D<float>(tex, i, j, k - 1)) / geo.dVoxelZ;
				}
				//sum += (az - ac)*(tex3D<float>(tex, i, j, k + 1) - tex3D<float>(tex, i, j, k - 1)) / 2 / geo.dVoxelZ;
				k = k + ku;
				ac = az;
				az += azu;
			}
	...
	case 4:
		for (unsigned long ii = 0; ii < Np; ii++) {
			...
			if (ax == aminc) {
				sum_step1 = (ax - ac)*tex3D<float>(tex, i, j, k);
				i = i + iu;
				ac = ax;
				ax += axu;
			}
			else if (ay == aminc) {
				sum_step1 = (ay - ac)*tex3D<float>(tex, i, j, k);
				j = j + ju;
				ac = ay;
				ay += ayu;
			}
			else if (az == aminc) {
				sum_step1 = (az - ac)*tex3D<float>(tex, i, j, k);
				k = k + ku;
				ac = az;
				az += azu;
			}
			aminc = fminf(fminf(ax, ay), az);
			if (ii > 0) {
				sum += (sum_step1 - sum_step0) / maxlength;
			}
...
int siddon_ray_projection(float* img, Geometry geo, const int diffselect, float** result, const GpuIds& gpuids, Blur blur) {
	...
	if (deviceCount == 0) {
		mexPrintf("Ax:Siddon_projection:GPUselect", "There are no available device(s) that support CUDA\n");
	}
	//
	// CODE assumes
	...
	if (!gpuids.AreEqualDevices()) {
		mexPrintf("Ax:Siddon_projection:GPUselect", "Detected one (or more) different GPUs.\n This code is not smart enough to separate the memory GPU wise if they have different computational times or memory limits.\n First GPU parameters used. If the code errors you might need to change the way GPU selection is performed.");
	}
	int dev;
		...
		for (unsigned int i = 0; i < noOfKernelCalls; i++) {
			for (dev = 0; dev < deviceCount; dev++) {
				...
				for (unsigned int j = 0; j < PROJ_PER_BLOCK; j++) {
					...
					computeDeltas_Siddon(geoArray[sp], proj_global, &uvOrigin, &deltaU, &deltaV, &source, blur);
					//Ray tracing!
				...
				kernelPixelDetector << <grid, block, 0, stream[dev * nStream_device] >> > (geoArray[sp], dProjection[(i % 2) + dev * 2], d_nCamU, d_nCamV, i, nangles_device, diffselect, texImg[dev]);
			}
			...
			if (!fits_in_memory&&sp > 0)
			{
				...
				for (dev = 0; dev < deviceCount; dev++)
				{
					...
					vecAddInPlace << <(geo.maxnCamU*geo.maxnCamV*projection_this_block + MAXTREADS - 1) / MAXTREADS, MAXTREADS, 0, stream[dev * 2] >> > (dProjection[(i % 2) + dev * 2], dProjection_accum[(i % 2) + dev * 2], (unsigned long)geo.maxnCamU*geo.maxnCamV*projection_this_block);
				}
...
void computeDeltas_Siddon(Geometry geo, int i, Point3D* uvorigin, Point3D* deltaU, Point3D* deltaV, Point3D* source, Blur blur) {
	...
	//if (blur.flag == 1) {
	//	Point3D* points;
	//	points = new Point3D[blur.num];  // 创建动态数组

	//	for (int k = 0; k < blur.num; ++k) {
	//		points[k].x = Pfinal.x + blur.DeltaX[i * blur.num];
	//		points[k].y = Pfinal.y + blur.DeltaY[i * blur.num];
	//		points[k].z = Pfinal.z + blur.DeltaZ[i * blur.num];
