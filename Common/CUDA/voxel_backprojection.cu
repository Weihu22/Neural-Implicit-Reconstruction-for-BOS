...
__global__ void matrixDiffTMultiply(const Geometry geo, float* image, float* imagediff, const int diffselect) {
	...
	case 1:
		if (indX == 0) {
			...
			imagediff[idx0] = image[idx0] * co0 + image[idx1] * co1;
		}
			...
			imagediff[idx0] = image[idx1] * co1 + image[idx2] * co2;
		}
		break;
	case 2:
		if (indY == 0) {
...
const int PROJ_PER_KERNEL = 1;  // 32 Number of 2D projections to be analyzed by a single thread. This can be tweaked to see what works best. 32 was the optimal value in the paper by Zinsser and Keck.
const int VOXELS_PER_THREAD = 1;  // 8 Number of voxels to be computed by s single thread. Can be tweaked to see what works best. 4 was the optimal value in the paper by Zinsser and Keck.
...
__global__ void kernelPixelBackprojection(const Geometry geo, float* image, int* d_nCamU,
	int* d_nCamV, const int currProjSetNumber, const int totalNoOfProjections, bool pseudo_mask, cudaTextureObject_t tex)
{
...
#pragma unroll
		for (colIdx = 0; colIdx < VOXELS_PER_THREAD; colIdx++)
		{
			...
			if (u >= d_nCamU[indAlpha] || v >= d_nCamV[indAlpha] || u<0 || v<0) break;

			float sample = tex3D<float>(tex, v, u, indAlpha);
			...
			if (pseudo_mask) {
				...
				voxelColumn[colIdx] += sample* weight * co;//
			}
...
int voxel_backprojection(float * projections, Geometry geo, const int diffselect, float* result, bool pseudo_mask, const GpuIds& gpuids) {
	...
	for (unsigned int img_slice = 0; img_slice < split_image; img_slice++) {
			...
			for (unsigned int proj_block_split = 0; proj_block_split < proj_split_overlap_number; proj_block_split++) {
				...
				for (dev = 0; dev < deviceCount; dev++) {
					...
					for (unsigned int i = 0; i < noOfKernelCalls; i++) {
						...
						kernelPixelBackprojection << <grid, block, 0, stream[dev*nStreamDevice] >> > (geoArray[img_slice*deviceCount + dev], dimage[dev], d_nCamU, d_nCamV, i, proj_split_size[proj_block_split], pseudo_mask, texProj[(proj_block_split % 2)*deviceCount + dev]);
		...
		dim3 grid((geo.nVoxelX + block.x - 1) / block.x,
			(geo.nVoxelY + block.y - 1) / block.y,
			(geo.nVoxelZ + block.z - 1) / block.z);
