...
__global__ void Ltx_kernel(float *out, float *W, float *x, int dim1, int dim2, int dim3, float lambda_sqrt) {
	...
	if (idx >= dim1 || idy >= dim2 || idz >= dim3) return;

	size_t x_size = 3 * dim1 * dim2 * dim3;
	// Helper lambda：安全读取 x，超界返回0
	auto safe_x = [=](size_t index) -> float {
		return (index < x_size) ? x[index] : 0.0f;
	};
	...
	GPUBufferPool& gpuPool)
{
	...
	// ================= 增加 =====================
// 用于实时 Producer/Consumer

	// Producer 循环外层，光流可以一直跑
	std::thread producer_thread([&]() {
		...
		while (true) { // 可以加终止条件
			...
			int buf = gpuPool.produce_idx.load(std::memory_order_acquire);
			for (int k = 0; k < geo.numCam; ++k) {
				...
				if (gpuPool.iref[k].empty() || gpuPool.idis[k].empty()) {
					mexPrintf("Warning: cannot load image cam %d frame %d\n", k, frame_idx);
			...
			for (int k = 0; k < geo.numCam; ++k)
			{
				gpuPool.d0[k].upload(gpuPool.iref[k], gpuPool.cvStreams[k]);
				gpuPool.d1[k].upload(gpuPool.idis[k], gpuPool.cvStreams[k]);
				...
				// Build hsv image
				cv::cuda::GpuMat _hsv[3], hsv, hsv8, bgr;
				_hsv[0] = angle;
				_hsv[1] = cv::cuda::GpuMat(angle.size(), CV_32F, cv::Scalar(1.0f));
				_hsv[2] = magnitude;

				cv::cuda::merge(_hsv, 3, hsv, gpuPool.cvStreams[k]);
				hsv.convertTo(hsv8, CV_8U, 255.0, 0.0, gpuPool.cvStreams[k]);
				// Display the results
				cv::cuda::cvtColor(hsv8, bgr, cv::COLOR_HSV2BGR, 0, gpuPool.cvStreams[k]);

				cv::Mat bgr_cpu;
				bgr.download(bgr_cpu, gpuPool.cvStreams[k]);
				//cv::imshow("Optical Flow Camera " + std::to_string(k), bgr);
			...
			// 等所有 camera
			for (int k = 0; k < geo.numCam; ++k)
				cudaStreamWaitEvent(gpuPool.flow_sync_stream,
					gpuPool.events[k], 0);

			// 同步流，保证 GPU 数据完整
			cudaStreamSynchronize(gpuPool.flow_sync_stream);
			
			cudaEventRecord(gpuPool.flow_frame_event[buf],
				gpuPool.flow_sync_stream);

			gpuPool.latest_ready_buf.store(buf, std::memory_order_release);

					
			// 标记最新光流版本
			gpuPool.flow_version.fetch_add(1, std::memory_order_release);

			// 切换 buffer
			gpuPool.produce_idx.store(buf ^ 1, std::memory_order_release);
			...
			mexPrintf("[Producer] frame %d total OF time = %.3f ms\n",
				frame_idx, of_ms);
		}
	});
	
	// ================== Consumer =====================
	// Reconstruction 取最新光流版本

	int last_consumed_version = 0;
	while (true) { // 可以加终止条件或迭代次数

		auto t0 = std::chrono::high_resolution_clock::now();
		int version = gpuPool.flow_version.load(std::memory_order_acquire);
	
		// 没有新帧
		if (version == last_consumed_version) {
			std::this_thread::sleep_for(std::chrono::milliseconds(10));
			continue;
		}
		int buf = gpuPool.latest_ready_buf.load(std::memory_order_acquire);
		if (buf < 0) continue;
		gpuPool.consume_idx.store(buf, std::memory_order_release);
		
		// 等待对应 Camera 的最新光流完成
		/*for (int k = 0; k < geo.numCam; ++k)
			cudaStreamWaitEvent(gpuPool.reconstructionStream, gpuPool.events[k], 0);*/
		cudaStreamWaitEvent(gpuPool.reconstructionStream, gpuPool.flow_frame_event[buf],0);
		//
		...
		precompute_KRinv_kernel << <blocks_cam, threads_cam, 0, gpuPool.reconstructionStream >> > (
			gpuPool.d_IMCam,
			gpuPool.d_RrCam,
			gpuPool.d_Zbc,
			gpuPool.d_Zpc,
	...
	// for build weight
		...
		while (iter < reconP.niter) {
			...
			while (iter < reconP.niter) {
				...
				if (*gpuPool.h_condition)
				{
					...
					remember = iter;
					iter--;
					mexPrintf("Orthogonality lost, restarting at iteration %d for frame %s. \n", iter, std::to_string(version));
					break;
				}
		...
		mexPrintf("[Consumer] frame %s total CGLS time = %.3f ms\n",
			std::to_string(version), cgls_ms);
		// ================== 保存 result 到 mat =====================
		 //---------- open mat ----------
		std::string file =
			"recon_flow_frame_" + std::to_string(version) + ".mat";

		MATFile* pmat = matOpen(file.c_str(), "w");
		if (!pmat) {
			mexPrintf("Error: cannot open %s\n", file.c_str());
			return;
		}
		...
		mxArray* mxRecon =
			mxCreateNumericArray(3, reconDims, mxSINGLE_CLASS, mxREAL);

		std::memcpy(mxGetData(mxRecon), gpuPool.h_x,
			gpuPool.voxelSize * sizeof(float));

		matPutVariable(pmat, "recon_x", mxRecon);
		mxDestroyArray(mxRecon);	
		matClose(pmat);

		mexPrintf("[GPU] Saved recon + flow to %s\n", file.c_str());
