...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, step_counter=None, mean_count=-1, perturb=False, align=-1, force_all_rays=False, dt_gamma=0, max_steps=1024):
        ...
        # bound = [float(bound)]*3 if isinstance(bound, (int, float)) else ([float(bound[0])]*3 if len(bound)==1 else [float(b) for b in bound])
        # bound = torch.tensor(bound, dtype=torch.float32, device=rays_o.device)
        ...
        rays = torch.empty(N, 3, dtype=torch.int32, device=rays_o.device) # id, offset, num_steps 表示第 i 条射线的所有点在 [offset, offset+count) 区间
...
class _composite_rays_train(Function):
    ...
    def forward(ctx, sigmas, rgbs, deltas, rays, T_thresh=1e-4):
        ...
        M = sigmas.shape[0] #所有射线上采样的点总数
        N = rays.shape[0]#射线数量（即图像中被采样的像素数量）

        weights_sum = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        depth = torch.empty(N, 3, dtype=sigmas.dtype, device=sigmas.device)
        image = torch.empty(N, 1, dtype=sigmas.dtype, device=sigmas.device)
    ...
    def backward(ctx, grad_weights_sum, grad_depth, grad_image):
        ...
        # NOTE: grad_image is not used now! It won't be propagated to sigmas.
        ...
        _backend.composite_rays_train_backward(grad_weights_sum, grad_depth, grad_image, sigmas, rgbs, deltas, rays, weights_sum, image, M, N, T_thresh, grad_sigmas, grad_rgbs)
