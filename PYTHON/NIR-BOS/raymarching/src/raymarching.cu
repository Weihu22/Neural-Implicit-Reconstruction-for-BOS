...
__global__ void kernel_march_rays_train(
    ...
    // locate 将指针移动到第 n 条光线的起点 rays_o[n] 和方向 rays_d[n]
    rays_o += n * 3;
    rays_d += n * 3;
...
__global__ void kernel_composite_rays_train_forward(
	...
	scalar_t d_x = 0, d_y = 0, d_z = 0;

    while (step < num_steps) {

        r += rgbs[0] * deltas[0];
        //d += sigmas[0] * deltas[0];
		d_x += sigmas[0] * deltas[0];
		d_y += sigmas[1] * deltas[0];
		d_z += sigmas[2] * deltas[0];
    ...
    // write
    weights_sum[index] = 0; // weights_sum
    depth[index * 3] = d_x;
	depth[index * 3 + 1] = d_y;
	depth[index * 3 + 2] = d_z;
    image[index] = r;
...
__global__ void kernel_composite_rays_train_backward(
    ...
    scalar_t T = 1.0f;
    const scalar_t r_final = image[0], ws_final = weights_sum[0];
    ...
    while (step < num_steps) {
       ...
       // r += rgbs[0] * deltas[0];
        ...
        // write grad_rgbs
        grad_rgbs[0] = grad_image[0] * deltas[0];
		grad_sigmas[0] = grad_depth[0] * deltas[0];
		grad_sigmas[1] = grad_depth[1] * deltas[0];
		grad_sigmas[2] = grad_depth[2] * deltas[0];
        // write grad_sigmas
...
void composite_rays_train_backward(const at::Tensor grad_weights_sum, const at::Tensor grad_depth, const at::Tensor grad_image, const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor deltas, const at::Tensor rays, const at::Tensor weights_sum, const at::Tensor image, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor grad_sigmas, at::Tensor grad_rgbs) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_depth.scalar_type(), "composite_rays_train_backward", ([&] {
...
__global__ void kernel_composite_rays(
    ...
    // scalar_t d = depth[0];
    scalar_t r = image[0];
	scalar_t d_x = depth[0];
	scalar_t d_y = depth[1];
	scalar_t d_z = depth[2];
    // accumulate 
    uint32_t step = 0;
    while (step < n_step) {
    ...
    weights_sum[0] = 0; // this is the thing I needed!
    depth[0] = d_x;
	depth[1] = d_y;
	depth[2] = d_z;
    image[0] = r;
