...
class HashEmbedder(nn.Module):
    ...
    def smoothstep_interp(self, x, voxel_min_vertex, voxel_max_vertex, voxel_embedds):
        '''
        x: B x 3
        voxel_min_vertex: B x 3
        voxel_max_vertex: B x 3
        voxel_embedds: B x 8 x F  (F 是 embedding 维度)
        '''
        # 归一化坐标
        weights = (x - voxel_min_vertex) / (voxel_max_vertex - voxel_min_vertex)  # B x 3

        # smoothstep 权重
        weights = weights * weights * (3.0 - 2.0 * weights)  # B x 3
    ...
    def forward(self, x, bound):
        ...
        for i in range(self.n_levels):
            ...
            x_embedded = self.smoothstep_interp(x, voxel_min_vertex, voxel_max_vertex, voxel_embedds)
