        ...
        int nrhs, mxArray const *prhs[])
{
    ...
    if (nrhs != 1) {
		mexPrintf( "MATLAB:getGpuName_mex:invalidNumInputs", "One input required.");
		return;
	} else if(nlhs > 1) {
		mexPrintf( "MATLAB:getGpuName_mex:maxlhs", "Too many output arguments.");
		return;
    }
    
    int iId = 0;
    if (mxIsDouble(prhs[0])) {
		mexPrintf( "MATLAB:getGpuName_mex:inputNotInt", "Input must be an integer.");
        return;
    } else {
