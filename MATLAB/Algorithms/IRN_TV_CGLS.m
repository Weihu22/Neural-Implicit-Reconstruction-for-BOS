function [x_outer,resL2,qualMeasOut]= IRN_TV_CGLS(proj,geo,niter,varargin)
...
[x0,diffselect,Axptype,QualMeasOpts,gpuids,lambda,niter_outer,gt,restart,flowmask,limits]=parse_inputs(proj,geo,varargin);
x=x0;
...
%     x=x0;
    ...
    prox_aux_1 = AxVal_fun(diffselect, x, geo,Axptype,gpuids); 
    prox_aux_2 = Lx(W, x)*sqrt(lambda);
    
    r_aux_1 = proj - prox_aux_1;
    r_aux_2 = -prox_aux_2;
    
    p_aux_1 = AtbVal_fun(diffselect, r_aux_1 , geo,gpuids);
    ...
    while iter<niter %this will be broken by a parameter
        ...
        q_aux_1 = AxVal_fun(diffselect, p, geo,Axptype,gpuids); 
        ...
        aux_Lx = Lx(W, x)*sqrt(lambda);
        resL2(iter)=im3Dnorm(aux,'L2');%+im3Dnorm(aux_Lx,'L2')
        ...
        s_aux_1 = AtbVal_fun(diffselect, r_aux_1 , geo,gpuids);
...
%% parse inputs'
function [x,diffselect,Axptype,QualMeasOpts,gpuids,lambda,niter_outer,gt,restart,flowmask,limits]=parse_inputs(proj,geo,argin)
opts=     {'init','initimg','diffd','axptype','qualmeas','gpuids','lambda','niter_outer','groundtruth','restart','flowmask','limits'};
defaults=ones(length(opts),1);

% Check inputs
nVarargs = length(argin);
if mod(nVarargs,2)
    error('BOSLAB:IRN_TV_CGLS:InvalidInput','Invalid number of inputs')
end
...
for ii=1:2:nVarargs
    ...
    if ~isempty(ind)
        ...
        error('BOSLAB:IRN_TV_CGLS:InvalidInput',['Optional parameter "' argin{ii} '" does not exist' ]);
    end
end
...
for ii=1:length(opts)
    ...
    if default==0
        ...
        if isempty(ind)
            error('BOSLAB:IRN_TV_CGLS:InvalidInput',['Optional parameter "' argin{jj} '" does not exist' ]);
        end
        val=argin{jj};
    end
...
%             end
            ...
            if isempty(x)
                error('BOSLAB:IRN_TV_CGLS:InvalidInput','Invalid Init option')
            end
        case 'niter_outer'
            if default
                niter_outer=10;
        ...
        case 'initimg'
            ...
            if exist('initwithimage','var')
                if isequal(size(val),geo.nVoxel')
                    ...
                    error('BOSLAB:IRN_TV_CGLS:InvalidInput','Invalid image for initialization');
                end
            end
            %  =========================================================================
            case 'diffd'
